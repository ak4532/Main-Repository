...
# Define column types for plotting
...
# Data Overview Page
    ...
    st.write("""
        ...
        For each flower, the dataset includes length and width of the sepals and petals.
        """)
    st.image(
...
# Exploratory Data Analysis (EDA) 
    ...
    eda_type = st.multiselect(
        ...
        ['Histograms', 'Box Plots', 'Scatter Plots', 'Count Plots']
    )

    if 'Histograms' in eda_type:
        st.subheader("Histograms - Visualizing Numerical Distributions")
        h_selected_col = st.selectbox("Select a numerical column for histogram:", num_cols)
        if h_selected_col:
            chart_title = f"Distribution of {h_selected_col.title().replace('_', ' ')}"
            if st.checkbox("Show by Species"):
                st.plotly_chart(px.histogram(
                    df, x=h_selected_col, color="species", title=chart_title, barmode='overlay'
                ))
            else:
    ...
    if 'Box Plots' in eda_type:
        ...
        if b_selected_col:
            ...
            st.plotly_chart(px.box(
                df, x="species", y=b_selected_col, title=chart_title, color="species"
            ))
    ...
    if 'Scatter Plots' in eda_type:
        ...
        if selected_col_x and selected_col_y:
            ...
            st.plotly_chart(px.scatter(
                df, x=selected_col_x, y=selected_col_y, color="species", title=chart_title
            ))
